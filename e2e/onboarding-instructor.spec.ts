...
test('sign up as new instructor via UI and reach dashboard', async ({ page }) => {
  await setupClerkTestingToken({ page })

  const email = `e2e-inst-${Date.now()}+clerk_test@example.com`
  ...
  await expect(page).toHaveURL(/\/instructor\/dashboard/, { timeout: 30_000 })
