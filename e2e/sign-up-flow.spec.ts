...
test('sign up as new agent via UI', async ({ page }) => {
  await setupClerkTestingToken({ page })

  const email = `e2e-agent-${Date.now()}+clerk_test@example.com`
  ...
  await expect(page).toHaveURL(/\/agent\/dashboard/, { timeout: 30_000 })
