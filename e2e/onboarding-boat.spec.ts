...
test('sign up as new boat via UI and reach dashboard', async ({ page }) => {
  await setupClerkTestingToken({ page })

  const email = `e2e-boat-${Date.now()}+clerk_test@example.com`
  ...
  await expect(page).toHaveURL(/\/boat\/dashboard/, { timeout: 30_000 })
