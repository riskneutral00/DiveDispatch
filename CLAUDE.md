...
The following are enforced by PostToolUse hooks and verified by /design-review:
...
`Design.md` (human intent) → `ui-ux-pro-max` (generates) → `MASTER.md` (technical spec) → `skins.ts` (runtime)

- **`Design.md`** lives in Vault (`Vaults/DiveDispatch/Design.md`). Matt owns it. Captures brand vision, palette, metaphor. Changes here require re-running `ui-ux-pro-max` to propagate.
- **`MASTER.md`** is the canonical technical spec. `/design-review` evaluates against it. Generated by `ui-ux-pro-max` — manual edits are overwritten on next generation.
...
- **`ui-ux-pro-max` = Design phase.** Generates `design-system/MASTER.md`
