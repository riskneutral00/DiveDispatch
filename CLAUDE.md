...
3. **`convex/seedData.ts` + `convex/parkedData.ts`** — the *dev replay layer*. Restored users are copied from (2) as TypeScript literals so `npm run seed:force` reproduces the hand-created state.
