...
| `AccessControlSection` | `@/components/profiles/access-control-section` | Allow/Block dive-center picker. Renders `<DiveCenter, Allow, Block>` grid with mutual exclusion + free-form (no allowlist + no blocklist = open). Exports `accessFromProfile`, `accessToPayload`, `deriveAccessMode`, `INITIAL_ACCESS_CONTROL`. Used standalone for DiveCenter access on resources, AND embedded inside `VenueEditDialog`. |
| `VenueCapabilitiesSection` | `@/components/profiles/venue-capabilities-section` | Single Venue surface with internal Pool / Dive Site sub-tabs. Each sub-tab is its own filtered `EntityCardList` + `VenueEditDialog`. Per-row contact (email/phone) is owned by each venue row, not by the operator org. Create-time defaults inherit latest-same-kind row → latest-any row → cross-role inheritance. |
| `VenueEditDialog` | `@/components/profiles/venue-edit-dialog` | Create/Edit dialog for individual venue rows. Accepts `lockedKind` so the parent sub-tab fixes the kind. Pool body: compact (basic + compressor). Dive-site body: segmented modal (basic / features / compressor / access tabs). Required at boundary: name, email, phone, location, maxDepth (+ maxCapacity for pool). Embeds `AccessControlSection`. |
...
| `GasMixFields` | `@/components/capabilities/gas-mix-fields` | Compound field group: "has compressor" checkbox + gas-mix selector + optional nitrox range. Used by `VenueEditDialog` (per-venue inline). Host-agnostic — any stakeholder that can offer gas fills consumes it. |
