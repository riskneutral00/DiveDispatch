...
| `VenueContactSection` | `@/components/profiles/venue-contact-section` | Venue Contact sub-tab. Reads/writes the operator's `organizations` row (business name, email, phone, address) — NOT venue rows. Multi-venue operators have a single business identity; individual venues are edited in the Pool/Dive Site tab. |
