...
| `VenueEditDialog` | `@/components/profiles/venue-edit-dialog` | Shared Create/Edit dialog for individual venue rows. Collects name, kind (`pool`/`dive_site`), location, depth, capacity, confined-capable flag, features[], compressor on-site fields, and embedded `AccessControlSection`. Consumed by `VenueCapabilitiesSection`. |
| `CompressorEditDialog` | `@/components/profiles/compressor-edit-dialog` | Shared Create/Edit dialog for compressor rows. Collects name, location (`fixed`/`boat`/`venue`), linked-boat or linked-venue selector, address (when fixed), gas mixes, optional nitrox min/max range. Consumed by `CompressorContactSection`. |
| `CompressorGasMixFields` | `@/components/profiles/compressor-gas-mix-fields` | Compound field group: "has compressor" checkbox + gas-mix selector + optional nitrox range. Used by `BoatFleetSection` (per-vessel inline) and `VenueEditDialog` (per-venue inline) — extract the third caller into a domain primitive before adding it. |
