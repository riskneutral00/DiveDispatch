...
| `RoleProfileForm` | `@/components/profiles/connected-role-forms` | Dynamic dispatcher. Reads `RoleConfig.contact` (default contact rendering) and `ROLE_SECTION_REGISTRY` (custom section components, e.g. agent's referral note + non-contact sections). Wires Convex mutations via `useEntityMutation(role)`. Passes canonical `BaseProfileSectionProps` downstream. Use `hasConnectedForm(roleKey)` to check availability. |
| `ROLE_SECTION_REGISTRY` | `@/components/profiles/role-section-registry` | `Record<RoleKey, Partial<Record<sectionId, ComponentType<BaseProfileSectionProps>>>>`. Component bindings only — non-contact sections + agent's custom contact wrapper. Default contact rendering for non-agent roles is config-driven via `RoleConfig.contact` in `src/lib/constants/roles.ts`. Covered by `tests/profileSectionRegistry.test.ts`. |
| `BusinessContactSection` | `@/components/profiles/business-contact-section` | Canonical Contact tab for every role. Props: `nameLabel` (omit for Instructor-style no-name forms), `schema`, `inheritFromOtherRoles?`, `createOverride?`, `afterSuccessfulSave?`, `extras?` (defaults + fromProfile + toPayload + render prop for role-specific add-ons like the agent referral note). The dispatcher in `connected-role-forms.tsx` constructs this from `RoleConfig.contact` for every role except agent (which keeps a custom wrapper for the referral note). |
...
| `use-entity-mutation` | Entity mutation hook keyed by `RoleKey` or `ClerkRole`. Returns `{ create, update, mine, idArg }` from `RoleConfig.api`. The single dynamic-role boundary where `Record<string, unknown>` typing is acceptable; sections recover strict types via `BaseProfileSectionProps` generics. Replaces the older `use-organizer-role-api`. |
