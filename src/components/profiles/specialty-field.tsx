...
function reorderToggleable(
  specs: AgencySpecialty[],
  agencyCode: string,
  mandatoryCodes: Set<string>,
): AgencySpecialty[] {
  const toggleable = specs.filter(
    (s) => !s.requiredForAOW && !mandatoryCodes.has(s.code),
  );
...
function useTwoRowOverflow(
  ...
  const registerMandatory = useCallback(
  ...
  const measure = useCallback(() => {
    ...
    if (remaining > 0) {
      const reservedBase =
        mandatoryWidth + (mandatoryWidth > 0 ? PILL_GAP : 0) + MORE_BTN_WIDTH;
      let used = 0;
      ...
      for (let i = r1; i < toggleableCount; i++) {
        ...
        if (afterAdd + PILL_GAP + reservedBase > containerWidth && used > 0)
          break;
        if (afterAdd + PILL_GAP + reservedBase > containerWidth && used === 0)
          break;
        used += needed;
      ...
      if (r2 === remaining) {
        ...
        for (let i = r1; i < r1 + remaining; i++) {
          ...
          if (!el) continue;
          const needed =
            checkUsed === 0 ? el.offsetWidth : PILL_GAP + el.offsetWidth;
          const withMandatory =
            checkUsed +
            needed +
            (reservedNoMore > 0 ? PILL_GAP + reservedNoMore : 0);
...
export function SpecialtyField({
  ...
  const toggleable = reorderToggleable(
    specialties,
    agencyCode || "PADI",
    mandatoryCodes,
  );
  const mandatorySpecs = specialties.filter(
    (s) => s.requiredForAOW || mandatoryCodes.has(s.code),
  );
