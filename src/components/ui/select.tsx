...
function TierSection({
  ...
  const tierMap = useMemo(() => {
    ...
    return new Map(
      items.map((opt) => [
        opt.id,
        scoreLanguageMatch(opt.languages ?? [], customerLanguages).tier,
      ]),
    );
...
export function Select({
  ...
  const isTiered =
    customerLanguages !== undefined && customerLanguages.length > 0;
  const tiers = isTiered
    ? splitInstructorTiers(options, customerLanguages)
    : null;
    ...
    if (!open) {
      if (e.key === "ArrowDown" || e.key === "Enter" || e.key === " ") {
        ...
        setFocusedIdx(
          Math.max(
            0,
            flatOptions.findIndex((o) => o.id === value),
          ),
        );
      }
      return;
    ...
    >
      <div ref={containerRef} className="relative">
        <button
          ...
          className={cn(
            "field-underline w-full text-body pl-0 pr-8 text-left",
          ...
          <span className="flex items-center justify-between gap-2">
            ...
            {selectedOption?.languages &&
              selectedOption.languages.length > 0 && (
                <LanguageFlags
                  languages={selectedOption.languages}
                  className="text-body flex-shrink-0"
                />
              )}
          </span>
