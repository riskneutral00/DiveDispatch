...
import { makeBookingResourcesSchema } from '@/lib/validation/bookingSchemas'
...
import { sortByPreference } from '@/lib/booking/sort-by-preference'
...
export function StepResources({ resources, details, dispatch, onValidityChange }: ResourcesStepProps) {
  const dates = useMemo(
  ...
  const preferredInstructorSlugs = useMemo(
    () => prefs?.preferredInstructorSlugs ?? [],
    [prefs],
  )

  // Notify parent when validity changes
  const resourcesSchema = useMemo(
    () => makeBookingResourcesSchema(details.activityType, needsPool),
    [details.activityType, needsPool],
  )
  useEffect(() => {
    if (!onValidityChange) return
    onValidityChange(resourcesSchema.safeParse(resources).success)
  }, [resources, resourcesSchema, onValidityChange])
  ...
  const instructorEntries = useMemo(
    () => sortByPreference(toPickerEntries(instructors ?? []), preferredInstructorSlugs),
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-2">
        <ResourcePicker
          ...
          preferredSlugs={preferredInstructorSlugs}
