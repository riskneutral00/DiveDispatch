...
export function PreferredEquipmentList({ slugs, onChange }: Props) {
  const [search, setSearch] = useState('')

  const allEquipment = useQuery(api.directory.listByRole, { role: 'Equipment' })
    ...
    <div className="space-y-3">
      ...
      {slugs.length === 0 ? (
        <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
          No preferred equipment providers added yet.
        </p>
      ) : (
              ...
              <GlassCard key={slug} padding="sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <GlassButton variant="destructive-ghost" size="sm" type="button" onClick={() => remove(index)} aria-label="Remove equipment provider">
                      <Trash2 size={16} />
                    </GlassButton>
                  </div>
                </div>
