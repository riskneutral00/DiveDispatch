...
export function PreferredCompressorList({ slugs, onChange }: Props) {
  const [search, setSearch] = useState('')

  const allCompressors = useQuery(api.directory.listByRole, { role: 'Compressor' })

  if (allCompressors === undefined) {
    return (
  ...
  const slugToEntry = Object.fromEntries(
    (allCompressors ?? []).map((e) => [e.slug, e]),
  )

  const trimmed = search.trim().toLowerCase()
  const searchResults = trimmed
    ? (allCompressors ?? []).filter(
        (e) =>
    ...
    <div className="space-y-3">
          ...
          >
            {searchResults.slice(0, 6).map((entry) => (
              ...
              >
                ...
                {entry.gasMixes && entry.gasMixes.length > 0 && (
                  <span
      ...
      {slugs.length === 0 ? (
        <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
          No preferred compressors added yet.
        </p>
      ) : (
              ...
              <GlassCard key={slug} padding="sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <GlassButton variant="destructive-ghost" size="sm" type="button" onClick={() => remove(index)} aria-label="Remove compressor">
                      <Trash2 size={16} />
                    </GlassButton>
                  </div>
                </div>
