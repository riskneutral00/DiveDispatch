...
import { getBarBorderColor, MULTIDAY_BORDER_VAR } from '../bar-styles'
...
describe('getBarBorderColor', () => {
  ...
  it('single-day Active uses status border color', () => {
    expect(getBarBorderColor('Active', false, STATUS_BORDER)).toBe(STATUS_BORDER)
  })
  ...
  it('multi-day Cancelled uses status border color', () => {
    expect(getBarBorderColor('Cancelled', true, STATUS_BORDER)).toBe(STATUS_BORDER)
  })
})
