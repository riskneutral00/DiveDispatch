 ...
 * If so, fires expireBooking once (idempotent — no-op if already Cancelled).
 ...
 * read-only, so expiry is split across query + mutation triggered here.
 */
export function useBookingWithExpiry(bookingId: Id<'bookings'>): {
  ...
  const expireBooking = useMutation(api.bookings.status.expireBooking)
  ...
  useEffect(() => {
    ...
    if (isExpiredBooking(booking)) {
      ...
      expireBooking({ bookingId }).catch(() => {
        // Idempotent — safe to ignore; booking may already be Cancelled
      })
    }
  }, [booking, bookingId, expireBooking])
