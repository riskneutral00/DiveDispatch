...
|----------|----------------|---------------|-----------|
| Main navigation (mobile bottom nav) | `nav.dashboard` / `nav.directory` / `nav.workspace` | **`/{slug}/{roleSlug}/workspace`** — Manage Roles, optional embedded profile (resource roles), equipment inventory (equipment role), **PreferencesEditor** | [`src/app/(dashboard)/[slug]/[roleSlug]/workspace/page.tsx`](../src/app/(dashboard)/[slug]/[roleSlug]/workspace/page.tsx), [`src/components/layout/mobile-bottom-nav.tsx`](../src/components/layout/mobile-bottom-nav.tsx) |
