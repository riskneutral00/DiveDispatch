...
3. Append TDD spec to `~/Desktop/RiskNeutral/Vaults/DiveDispatch/Product/TODO.md` under `### Code Health Hardening`:
...
> NOTE: TODO.md at the vault path is the write target for H-specs. The `.tickets/` mirror at project root is auto-generated by `/board sync` — never edit it directly.

4. Create ticket via `/board create` referencing the H-spec number and the specific failing test file.
