...
## Skills must be hook-triggered
Every skill must be wired to a hook that triggers automatically. Skills requiring manual `/skillname --flag` calls are dead code. Overstory is deprecated.
...
Run `bash scripts/validate-merge.sh <branch>` before merging any autonomous branch to main. It checks for duplicate tests, zero-value tests, banned patterns, tsc errors, and test failures. Individual commits are preserved — don't squash good work. The pre-commit hook catches trash at write time; validate-merge catches anything that slipped through.
