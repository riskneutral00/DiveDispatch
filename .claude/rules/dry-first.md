...
- **Entity-row slugs:** every entity-role table (compressors, venues, diveCenters, boats, equipment) has `slug: v.string()` + `by_slug` index. Validators in `convex/lib/entitySlug.ts`: `assertEntitySlug` (non-empty, kebab-case, 3-64 chars), `assertEntitySlugUnique`, `deriveDefaultEntitySlug(table, userSlug)`. See `[[entity-row-slug]]` vault entity.
