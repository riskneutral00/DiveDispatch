...
// ─── 8: edit creates audit entry with diff ────────────────────────────────────

describe('edit creates audit entry with diff', () => {
  it('re-submit with new dates logs edited entry with old/new dates in diff', async () => {
    const t = makeT()
...
// ─── 9: diff captures changed fields only ─────────────────────────────────────

describe('diff captures changed fields only', () => {
  it('changing only startDate: diff has startDate, not endDate', async () => {
    const t = makeT()
    ...
    // Re-submit: only startDate changes, endDate stays the same
    await t.withIdentity({ tokenIdentifier: 'clerk|dc-test' }).mutation(
      api.bookings.create.submitToDraft,
      {
        bookingId,
...
// ─── 10: audit entries are immutable ──────────────────────────────────────────
