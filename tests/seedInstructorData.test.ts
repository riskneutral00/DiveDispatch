...
describe('ALL_INSTRUCTORS seed data', () => {
  it('has at least 10 instructors', () => {
    expect(ALL_INSTRUCTORS.length).toBeGreaterThanOrEqual(10)
  })

  it('all instructors have unique slugs', () => {
    const slugs = ALL_INSTRUCTORS.map((s) => s.user.slug)
    expect(new Set(slugs).size).toBe(slugs.length)
  })

  it('all instructors have unique emails', () => {
    const emails = ALL_INSTRUCTORS.map((s) => s.user.email)
